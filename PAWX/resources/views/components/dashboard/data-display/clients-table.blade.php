                ...
                <table class="table w-full mt-10">
                    ...
                    @forelse ($clients as $client)
                        <tr class="hover:bg-pawx-brown/5 text-pawx-brown/80 cursor-pointer text-sm  text-left"
                            ...
                            <td class="px-2">
                                <a href="{{ route($rolePrefix . '.clients.show', $client->client->id) }}"
                            ...
                            @if(auth()->user()->hasRole('admin'))
                                <td class="px-2">
                                    <form method="POST"
                                          action="{{ route($rolePrefix . '.clients.destroy', $client->client->id) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-500 hover:text-red-800">Delete
                                        </button>
                                    </form>
                                </td>
                            @endif
